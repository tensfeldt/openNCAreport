...
- `parameter_label` - represents the parameter name label used to report parameters with PDS codelist. If missing, there's not a corresponding parameter and parameter name may represent a "placeholder" parameter.
- `cdisc_label` - represents the cdisc parameter name label used to report the parameters with CDISC codelist. If missing, there's not a corresponding parameter and parameter name may represent a "placeholder" parameter.
- `callfun` - represents the function that will be used to generate the parameters
- `regex` - regular expression that is used to identify the parameter
- `unit_class` - unit class that the parameter belongs to and is used for unit conversion
- `valid_models` - represents which model/dosing type the parameter belongs to for computation
- `display_list_models` - represents which model/dosing type the parameter will be displayed as an output for
- `predecessors` - list of parameters that the specified parameters belong to
...
For more advanced users, you may use the code above to make updates to the dependency list, and then it's possible to build the new version back into the package via:
